...
instance {α : Type u} [ToSortLevel.{u + 1}] [ToExpr α] : ToExpr (Option α) :=
...
instance {α : Type u} [ToSortLevel.{u + 1}] [ToExpr α] : ToExpr (List α) :=
  let type := toTypeExpr α
  let lvl := ToSortLevel.toSortLevel.{u + 1}
...
instance {α : Type u} [ToSortLevel.{u + 1}] [ToExpr α] : ToExpr (Array α) :=
  let type := toTypeExpr α
  let lvl := ToSortLevel.toSortLevel.{u + 1}
