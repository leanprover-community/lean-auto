...
theorem LamWF.interp_app_bvarLift_bvar0
  ...
  show interp lval (pushLCtx argTy lctx) (pushLCtxDep x lctxTerm) (bvarLift t wft) x = interp lval lctx lctxTerm wft x
  rw [← LamWF.interp_bvarLift]
...
theorem LamTerm.maxEVarSucc_etaExpand1 : (etaExpand1 s t).maxEVarSucc = t.maxEVarSucc := by
  show t.bvarLift.maxEVarSucc.max 0 = t.maxEVarSucc
  rw [maxEVarSucc_bvarLift]; rw [Nat.max, Nat.max_zero_right]
...
theorem LamTerm.maxEVarSucc_extensionalizeEq :
  ...
  case base b =>
    cases b <;> try rfl
    case eq s =>
      show (extensionalizeEqF s (bvar 1) (bvar 0)).maxEVarSucc = 0
      rw [maxEVarSucc_extensionalizeEqF]; rfl
  case app s fn arg =>
    cases fn <;> try rfl
    case base b =>
      cases b <;> try rfl
      case eq s =>
        show (extensionalizeEqF s arg.bvarLift (bvar 0)).maxEVarSucc = Nat.max 0 arg.maxEVarSucc
        rw [maxEVarSucc_extensionalizeEqF, maxEVarSucc_bvarLift]; apply Nat.max_comm
    case app _ fn' arg' =>
      cases fn' <;> try rfl
      case base b =>
        cases b <;> try rfl
...
| lctxTy, lctxTerm, wfArg, .ofApp argTy' HFn HArg =>
  ...
  by dsimp [LamWF.interp]; dsimp at IHFn; dsimp at IHArg; simp [IHFn, IHArg]; rfl
...
theorem LamWF.interp_resolveImport
  ...
  | .ofLam s hwf => by
    ...
    rw [LamWF.interp_resolveImport _ _ hwf]; rfl
...
theorem LamTerm.maxEVarSucc_neSymm?
  ...
  | .app _ (.base .not) (.app s (.app _ (.base (.eq _)) lhs) _), Eq.refl _ => by
    simp [maxEVarSucc, Nat.max, Nat.max_zero_left, mkEq, mkNot]
    apply Nat.max_comm
