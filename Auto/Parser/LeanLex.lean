...
inductive LexResult where
  | done       : σ → LexResult
  | incomplete : LexResult
  | malformed  : LexResult
deriving Inhabited

variable {σ : Type}

def LexResult.toString [ToString σ] : LexResult σ → String
| .done s     => s!"LexResult.done {s}"
| .incomplete => "LexResult.incomplete"
| .malformed  => "LexResult.malformed"

instance [ToString σ] : ToString (LexResult σ) where
...
def ERE.toADFA (e : ERE) : ADFA Char :=
  ...
  let nfa := (ERE.comp #[.star .startp, e]).toNFAAux cg
...
--   character in `s` such that (there exists a string (starting with
--   this character and matches `e`)) (thus the name `eager`).
--   Then, it returns the longest match (as a substring) if there is
--   one, and returns `none` otherwise.
-- We prepend `s` with the "beginning of string" group
--   and append to `s` the "end of string" group
def ERE.ADFALexEagerL (a : ADFA Char) (s : Substring) (short := false) (strict := false) : Lexing.LexResult (Substring × HashSet String) := Id.run <| do
  ...
  let mut state : Nat := a.dfa.move 0 beginString
  ...
  match es with
  | .some (e, finalstate) =>
    if finalstate >= a.dfa.attrs.size then
      panic!"ERE.ADFALex :: {finalstate} is invalid for {a.dfa}"
    else
      let attrs := a.dfa.attrs[finalstate]!
      return .done (⟨s.str, b, e⟩, attrs)
  | .none =>
...
instance : ToString (Lexing.LexResult (Substring × HashSet String)) where
  toString s :=
    have : ToString (Substring × HashSet String) :=
      ⟨fun (sub, hs) => s!"{sub.toString} {hs.toList}"⟩
    toString s
...
private def testit (a : ADFA Char) (s : Substring) (short:=false) (strict:=false) : IO Unit := do
  ...
  match r with
  | .done (sub, hset) => IO.println sub; IO.println hset.toList
